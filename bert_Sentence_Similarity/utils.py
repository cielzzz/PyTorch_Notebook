...
def validate(model, dataloader):
    ...
    return epoch_time, epoch_loss, epoch_accuracy, roc_auc_score(all_labels, all_prob), f1_score(all_labels, all_prob)
...
def test(model, dataloader):
    ...
    return batch_time, total_time, accuracy, roc_auc_score(all_labels, all_prob), f1_score(all_labels, all_prob)
...
def train(model, dataloader, optimizer, epoch_number, max_gradient_norm):
    ...
    for batch_index, (batch_seqs, batch_seq_masks, batch_seq_segments, batch_labels) in enumerate(tqdm_batch_iterator):
        ...
        #对抗训练
        fgm.attack()
        loss_adv = model(seqs, masks, segments, labels)
        loss_adv.backward()
        fgm.restore()
...
class PGD():
    ...
    def attack(self, epsilon = 1., alpha = 0.3, emb_name = 'word_embedding', is_first_attack = False):
        # emb_name换成模型中embedding的参数名
        for name, param in self.model.named_parameters():
            if param.requires_grad and emb_name in name:
                if is_first_attack:
    ...
    def project(self, param_name, param_data, epsilon):
        ...
        return self.emb_backup[param] + r
